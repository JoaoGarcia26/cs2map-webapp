...
std::string utils::get_ipv4_address(config_data_t& config_data)
{
    ...
    // Prefer explicit public host if provided (can be domain or IP)
    if (!config_data.m_public_ip.empty())
        return config_data.m_public_ip;
    ...
    if (!pipe)
    {
        ...
        // fallback to local_ip from config if available
    ...
    // Final fallback
    if (ip_address.empty() && !config_data.m_local_ip.empty())
        return config_data.m_local_ip;
